...
//                    exit();
		...
		if(empty($_POST['item_id'])){
			showDialog(Language::get('store_activity_choose_goods'),'index.php?act=store_promotion_booth&op=adposition_apply');
		}
		...
		foreach ($_POST['item_id'] as $key=>$item_id){
			...
			if(!in_array($item_id,$ids)){
				$input	= array();
				$input['adp_id']	= $adp_id;
				$input['item_name']	= $_POST['item_name'][$key];
				$input['item_id']	= $item_id;
                                $input['item_cate']	= $_POST['item_cate'][$key];
