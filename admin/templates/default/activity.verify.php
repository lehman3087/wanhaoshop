...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        <li><a href="index.php?act=activity&op=index"><span><?php echo $lang['nc_manage'];?></span></a></li>
        <li><a  class="current" href="<?php echo urlAdmin('activity', 'activity', array('verify' => 'waitverify'));?>"><span>等待审核</span></a></li>
      ...
      <tbody>
        <tr>
          ...
          <td><select name="activity_type">
              <option value="0" <?php if (!$_GET['activity_type']){echo 'selected=selected';}?>>活动类型</option>
              <option value="10" <?php if ($_GET['activity_type'] == 10){echo 'selected=selected';}?>>报名活动</option>
              <option value="1" <?php if ($_GET['activity_type'] == 1){echo 'selected=selected';}?>>商品</option>
            </select>
          </td>
          <th>发起人</th>
          <td>
              ...
              <select name="funder_type">
                  <option> 请选择</option>
              <option value="1" <?php if ($_GET['funder_type']==1){echo 'selected=selected';}?>>商管</option>
               <option value="2" <?php if ($_GET['funder_type']==2){echo 'selected=selected';}?>>商户</option>
            </select>
          </td>
  ...
  <form id="listform" action="index.php" name="activityform" method='post'>
    <input type="hidden" name="act" value="activity" />
...
<!--          <td class="align-center">
          	...
          	<a href="index.php?act=activity&op=detail&id=<?php echo $v['activity_id'];?>"><?php echo $lang['activity_index_deal_apply'];?></a>
                <?php }else{ ?>
                <a href="index.php?act=activity&op=member&id=<?php echo $v['activity_id'];?>&act_name=<?php echo $v['activity_title'];?>">查看报名</a>
                <?php } ?>
          </td>-->
        </tr>
        <?php } ?>
...
function getId() {
    var str = '';
    $('#activityform').find('input[name="activity_id[]"]:checked').each(function(){
...
function goods_verify(ids) {
    _uri = "<?php echo ADMIN_SITE_URL;?>/index.php?act=activity&op=activity_verify&id=" + ids;
    CUR_DIALOG = ajax_form('activity_verify', '审核报名活动', _uri, 350);
}
