    ...
    <table class="tb-type1 noborder search">
      <tbody>
        <tr>
          ...
          <td><select name="apply_type">
              <option value=""><?php echo $lang['nc_please_choose'];?>...</option>
              <?php if(!empty($output['apply_type_array']) && is_array($output['apply_type_array'])){ ?>
              <?php foreach($output['apply_type_array'] as $k => $v){ ?>
              <option value="<?php echo $k;?>" <?php if($k==$_GET['apply_type']) { echo 'selected'; }?>><?php echo $v;?></option>
              <?php } ?>
              <?php } ?>
            </select></td>
