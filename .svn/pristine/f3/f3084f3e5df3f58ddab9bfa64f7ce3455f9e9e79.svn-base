...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        <li><a  href="<?php echo urlAdmin('decoration', 'works', '');?>"><span>全部需求</span></a></li>
        
        <li><a class="current" href="<?php echo urlAdmin('decoration', 'works', array('type' => 'waitverify'));?>"><span>等待审核</span></a></li>
      </ul>
    </div>
  </div>
  <div class="fixed-empty"></div>
  <form method="get" name="formSearch" id="formSearch" style="display: none">
    ...
    <table class="tb-type1 noborder search">
      <tbody>
        ...
        <tr>
          ...
          <td style="display:none;"><div id="ajax_brand" class="ncsc-brand-select w180">
              <div class="selection">
    ...
    <table class="table tb-type2">
      <thead>
        ...
        <tr class="thead">
          ...
          <th>面积&装修类型&风格&房屋类型</th>
          <th class="w72 align-center">预算（万）</th>
      ...
      <tbody>
        ...
        <tr class="hover edit">
          <td class="w24"><input type="checkbox" name="id[]" value="<?php echo $v['dw_id'];?>" class="checkitem"></td>
          <td><i class="icon-plus-sign" nctype="ajaxGoodsList" data-comminid="<?php echo $v['dw_id'];?>" style="cursor: pointer;"></i></td>
          <td><?php echo $v['dw_id'];?></td>
          ...
          <td class><p><?php echo $v['gc_name'];?></p>
            <p class="goods-brand"><?php echo $v['dw_area'].'&'.$v['dw_category'].'&'.$v['dw_style'].'&'.$v['dw_house_type'];?></p></td>
          <td class="align-center"><?php echo $lang['currency'].$v['dw_budget']?></td>
          
          <td class="align-center"><p><?php echo $output['verify'][$v['dw_verify']];?></p>
            <?php if ($v['goods_verify'] == 0) {?>
            <p><?php echo $v['goods_verifyremark'];?></p>
            <?php }?></td>
          <td class="w48 align-center"><a href="<?php echo urlShop('goods', 'index', array('goods_id' => $output['storage_array'][$v['goods_commonid']]['goods_id']));?>" target="_blank"><?php echo $lang['nc_view'];?></a>&nbsp;|&nbsp;<a href="javascript:void(0);" onclick="goods_verify(<?php echo $v['dw_id'];?>);">审核</a></td>
        </tr>
...
function goods_verify(ids) {
    _uri = "<?php echo ADMIN_SITE_URL;?>/index.php?act=decoration&op=dw_verify&id=" + ids;
    CUR_DIALOG = ajax_form('dw_verify', '审核需求', _uri, 350);
}
