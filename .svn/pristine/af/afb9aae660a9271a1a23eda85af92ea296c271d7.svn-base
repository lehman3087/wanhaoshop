...
//		Tpl::output('show_page',$page->show());
	...
	public function album_pic_listOp(){
		...
		$model_album = Model('album');
                ...
                $albumId=!empty($_SESSION['store_id'])?$_SESSION['store_id']:0;
		$param = array();
		$param['aclass_id']	= intval($_GET['id']);
		$param['album_pic.store_id']	= $albumId;
		if($_GET['sort'] != ''){
			switch ($_GET['sort']){
				case '0':
					$param['order']		= 'upload_time desc';
	...
	public function pic_listOp(){
		...
		$page->setStyle('admin');
                ...
                $storeid=!empty($_SESSION['store_id'])?$_SESSION['store_id']:0;
		/**
		 * 实例化相册类
		 */
		$model_album = Model('album');
		 ...
		 */
		$param = array();
		$param['album_pic.store_id']	= $storeid;
		if(!empty($_GET) && $_GET['id'] != '0'){
			...
			$cparam['value']		= array(intval($_GET['id']),$storeid);
		 ...
		 */
		$param = array();
		$param['album_aclass.store_id']	= $storeid;
