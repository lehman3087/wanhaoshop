...
$(document).ready(function(){
    ...
    $('#btn_apply_agreement_next').on('click', function() {
     ...
     $('#btn_apply_decoration_agreement_next').on('click', function() {
        if($('#input_apply_agreement').prop('checked')) {
            window.location.href = "index.php?act=decoration_join&op=step1";
        } else {
            alert('请阅读并同意协议');
        }
    });
  ...
  <div id="apply_agreement" class="apply-agreement">
    ...
    <div class="bottom">
		...
		<a id="btn_apply_agreement_next" href="javascript:;" class="btn">店铺入驻</a>
                <a id="btn_apply_decoration_agreement_next" href="javascript:;" class="btn">装修公司入驻</a>
	</div>
  </div>
