...
class show_storeControl extends BaseStoreControl {
    ...
    public function show_shareOp() { 
                //判断是否为导航页面
                //$store_sns_tracelog = Model('store_sns_tracelog');
                //$store_tracelog_info = $store_sns_tracelog->getStoreNavigationInfo(array('sn_id' => intval($_GET['sn_id'])));
                $model_stracelog = Model('store_sns_tracelog');
		$stracelog_info = $model_stracelog->getStoreSnsTracelogInfo(array('strace_id' => 2));
                
                
                
                
                if (!empty($stracelog_info)){
                  //  var_dump($stracelog_info);
                    Tpl::output('store_tracelog_info',$store_tracelog_info);
                    Tpl::showpage('share_article');
                }
    }
