...
class wxpay{
    ...
    public function __construct() {
       ...
       // $this->sslcert_path = dirname(__FILE__).DIRECTORY_SEPARATOR.'cert'.DIRECTORY_SEPARATOR.'apiclient_cert.pem';
         $this->sslkey_path ='/var/nginx/whao/wanhaoshop/mobile/api/payment/wxpay/cert/apiclient_cert.pem';
       ...
       // var_dump('11111');
        ...
        $this->sslkey_path ='/var/nginx/whao/wanhaoshop/mobile/api/payment/wxpay/cert/apiclient_key.pem';
    }
    ...
    private function dopwxpay($openid, $order_sn){
    ...
    public function getpWpay($pay_sn){
...
//        $this->parameters['attach']=$order_type.'|'.$pay_sn;
      ...
      //  $this->getSign($this->parameters);
       // $this->parameters['openid']=$openid;
       // $this->parameters['sign']='';
        $this->parameters['prepay_id'] = $this->getPrepayId();
        $this->setPrepayId($this->parameters['prepay_id']);
...
eot;
    ...
    public function getAppParameters()
    {
        ...
        $appApiObj['partnerid']=$this->parter_id;
        $timeStamp = time();
        ...
        //var_dump($appApiObj);
        $appApiObj["sign"] = $this->getLowercaseSign($appApiObj);
        output_data($appApiObj);
       // var_dump($appApiObj);
       // $this->parameters = json_encode($appApiObj);
       // var_dump(json_encode($appApiObj));
       // return json_encode($appApiObj);
    }
     ...
     *  作用：生成签名
     */
    public function getLowercaseSign($Obj)
    {
        foreach ($Obj as $k => $v)
        {
            $Parameters[$k] = $v;
