...
class storeModel extends Model {
    ...
    public function getStoreInfo($condition) {
        ...
        if(!empty($store_info)) {
            ...
            $store_info['store_logo'] = getStoreLogo($store_info['store_logo']);
            //店铺评价
...
//    		}
    ...
    private function getWorkBy($store_array) {
    	...
    	foreach ($store_array as $value) {
    		...
    		$store_array[$value['store_id']]['search_list_works'] = $model->table('designer_work')->field($field)->where(array('sn_store_id'=>intval($value['store_id']),'sn_if_show'=>1))->order('sn_collection_count desc')->limit(8)->select();
                $condition['sn_store_id']=$value['store_id'];
                $count = $model->table('designer_work')->where($condition)->count();
