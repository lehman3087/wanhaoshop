...
class decoration_joinControl extends BaseHomeControl {
    ...
    public function step0Op() {
        ...
        $decoration_info = $model_document->getOneByCode('decoration');
        
        
        Tpl::output('step', '0');
        Tpl::output('sub_step', 'step0');
        
        if($_GET['role']=='designer'){
            Tpl::output('agreement', $decoration_info['doc_content']);
        }else{
            Tpl::output('agreement', $document_info['doc_content']);
        }
        
       
        Tpl::showpage('decoration_joinin_apply');
        exit;
    }
...
//        $param['paying_amount'] = floatval($grade_list[$_POST['sg_id']]['sg_price'])*$param['joinin_year']+floatval($param['sc_bail']);
        ...
        @header('location: index.php?act=decoration_join');
...
//            array("input"=>$param['sc_id'], "require"=>"true","message"=>"店铺分类不能为空"),
    ...
    private function show_join_message2($message, $btn_next = FALSE, $step = '3') {
