...
<!--          <th><?php echo $lang['rec_ps_content'];?></th>
      ...
      <tbody>
        ...
        <tr class="hover edit">
          ...
          <td><?php if($v['rec_app']==1){ echo str_replace(array(1,2,3,4),array('单图','文章','商户推荐','平台活动'),$v['rec_content_type']);}else{ echo 'wap推荐位';}?>
          
              <?php if ($v['rec_content_type'] == 3){echo !empty($v['content']['body']['imageurl']) ? '[已选择]' : '[未选择]';}?>
          </td>
<!--          <td class="picture"><?php 
          ...
          <td class="w180 align-center">
              <a href="index.php?act=rec_position&op=rec_edit&rec_id=<?php echo $v['rec_id'];?>"><?php echo $lang['nc_edit'];?></a> 
              | <a onclick="if(confirm('<?php echo $lang['nc_ensure_del'];?>')){return true;}else{return false;}" href="index.php?act=rec_position&op=rec_del&rec_id=<?php echo $v['rec_id'];?>"><?php echo $lang['nc_del'];?></a> 
