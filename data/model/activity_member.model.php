...
class activity_memberModel extends Model {

    public function __construct(){
        parent::__construct('activity_member');
    }
    ...
    public function getMemberInfo($condition, $field = '*', $master = false) {
        return $this->table('activity_member,member')->field($field)->on("activity_member.act_m_user_id=member.member_id")->where($condition)->master($master)->find();
    }
...
//    }
    ...
    public function getMemberList($condition = array(), $field = '*', $page = 0, $order = 'member_id desc', $limit = '') {
       return $this->table('activity_member,member')->on("activity_member.act_m_user_id=member.member_id")->where($condition)->page($page)->order($order)->limit($limit)->select();
    }
     ...
     * @param string $order
     */
    public function getMemberApplyList($condition = array(), $field = '*', $page = 0, $order = 'act_m_id desc', $limit = '') {
       return $this->table('activity_member,activity')->on("activity_member.act_m_apply_act_id=activity.activity_id")->where($condition)->page($page)->order($order)->limit($limit)->select();
    }
